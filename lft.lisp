...
(defun do-trial (course vehicle file)  
  (let* ((subgoal nil)
	...
	(setf (color vehicle) 'RED)
	(setf (color vehicle) 'GREEN)) 
...
(defun draw-vehicle (vehicle-x-posi vehicle-y-posi x y f)
  (when (>= x y)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi)
			    (list (+ vehicle-x-posi x) (+ vehicle-y-posi y)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi)
			    (list (- vehicle-x-posi x) (+ vehicle-y-posi y)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi) 
			    (list (+ vehicle-x-posi x) (- vehicle-y-posi y)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi) 
			    (list (- vehicle-x-posi x) (- vehicle-y-posi y)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi) 
			    (list (+ vehicle-x-posi y) (+ vehicle-y-posi x)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi) 
			    (list (- vehicle-x-posi y) (+ vehicle-y-posi x)) :color 'light-gray)
    (add-line-to-exp-window (list vehicle-x-posi vehicle-y-posi) 
			    (list (+ vehicle-x-posi y) (- vehicle-y-posi x)) :color 'light-gray)
  ...
  (let ((subgoal nil) (yposi *disp-hi*) (subg) (txt-c) (offset 2) (xposi) (next) (flag nil)
    ...
    (dotimes (i (length course))
	  ...
	  (add-text-to-exp-window 
	   :text "." :width 1 :x xposi :y yposi :color 'cyan))
      (when subg
	...
	(when *debg*
	  (if (car courseg)
		...
		(if (<= i 50)
		    (add-line-to-exp-window (list xposi yposi)
					    (list ixposi *disp-hi*) :color 'black))
		(add-line-to-exp-window (list (car (car courseg)) (- yposi 50))
					(list xposi yposi) :color 'black))
	      (add-line-to-exp-window (list xposi yposi)
				      (list (car (car (reverse course))) 1) :color 'black)))
	(when  (and (<= yposi (- *vehicle-y-posi* (* (abs (- xposi next)) 3))) ;nearest goal
	  ...
	  (if (equal subg 'L)
		...
		(add-text-to-exp-window 
		 :text "G" :width 1 :x (- xposi (+ *vehicle-radius* offset)) :y yposi :color txt-c))
	      (progn
		(push (+ xposi *vehicle-radius*) subgoal)	      
		(add-text-to-exp-window 
		 :text "G" :width 1 :x (+ xposi (+ *vehicle-radius* offset)) :y yposi :color txt-c))))))
...
(defun show-error-character ()
  (dotimes (i 50)
    (let ((pos (* i 10)))
      (add-text-to-exp-window :text "O" :x pos :y 0 :color 'YELLOW)
      (add-text-to-exp-window 
       :text "O" :x pos :y (- *disp-hi* 10) :color 'YELLOW)
      (add-text-to-exp-window :text "O" :x 0 :y pos :color 'YELLOW)
      (add-text-to-exp-window :text "O" :x 490 :y pos :color 'YELLOW))))

(defun show-error-line ()
  (add-line-to-exp-window (list 5 (- *disp-hi* 5)) (list 5 5) :color 'light-gray)
  (add-line-to-exp-window (list (- *disp-wi* 5) 5)  (list 5 5) :color 'light-gray)
  (add-line-to-exp-window (list (- *disp-wi* 5)  (- *disp-hi* 5))  (list (- *disp-wi* 5) 5) :color 'light-gray)
  (add-line-to-exp-window (list 5 (- *disp-hi* 0))  (list (- *disp-wi* 5)  (- *disp-hi* 0)) :color 'light-gray))


;another reward method
(defun trial-reward (subgoal)
  (unless (or (equal 'press-space-m (chunk-slot-value-fct (act-r-buffer-chunk (buffer-instance 'goal)) 'previous-rule))
	      (equal 'press-space-a (chunk-slot-value-fct (act-r-buffer-chunk (buffer-instance 'goal)) 'previous-rule))
	      (equal 'press-space (chunk-slot-value-fct (act-r-buffer-chunk (buffer-instance 'goal)) 'previous-rule)))
    (if (or (equal *prev-performance* 0) (equal subgoal *vehicle-x-posi*))
	(trigger-reward 10)	
	(trigger-reward 0))))
;      (progn
	;(if (equal *move-distance* 2)
	 ;   (trigger-reward 1)
	  ;  (trigger-reward 0))      
	;(setf *move-distance* 0))))
...
(defmethod device-hold-finger ((win rpm-window) hand finger)
  (when *keyable*
    (setf *keyable* nil) 
    (when (equal finger 'index)
      (setf *key* 'left))
    (when (equal finger 'ring)
      (setf *key* 'right))))
...
(defun run-block (ac mc file)
  ...
  (setf *window*
	(open-exp-window "Line Tracing Task" 
			 ...
			 :width *disp-wi* 
			 :height *disp-hi*))
  (let ((temp nil) vehicle)
    (setf vehicle (add-text-to-exp-window 
		   :text "'" :width 1 :color 'RED :x *vehicle-x-posi* :y *vehicle-y-posi*))
     ...
     *refresh* #'(lambda ()
		   (setf temp (subseq *course* 0 (/ *disp-hi* *interval-course*)))		   
		   (do-trial temp vehicle file)
...
(defun run-block-init (ac mc)
  (init)
  (prepare-output-file (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname)))
  (reset) ;initialize
  (pdisable to.auto2)	      
  (pdisable to.manual2)
  (setf *run-time* 0) 
  (setf *course* (create-course (prepare-next 1 '((0 R)) (create-random-course *course-length*))))
  (run-block ac mc (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname))))
...
(defun repeat-experiment2 (n ac mc)
  (init)
  (dotimes (i n)
    (run-block-init ac mc)))
...
(defun run-experiment (n)
  ...
  (dotimes (k n)
	...
	(setf *course* (create-course (prepare-next 1 '((0 R)) (create-random-course *course-length*))))
	(run-block (+ i 3) (+ j 3) (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname)))))))

;run-experimen corresponds Experiment 2 that manupulates Ca/Cm.
(defun run-experiment-order (n)
  (init)
  (prepare-output-file (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname)))
  (dotimes (k n)
    (setf *run-num* k)
    (reset) ;initialize
    (let ((order-list (random-order (combine-capabilities '(0 1 2 3 4) nil)  nil)))
      (dotimes (i 25)
	(setf *run-time* 0) 
	(setf *vehicle-posi* 320)
	(pdisable to.auto2)	      
	(pdisable to.manual2)
	(setf *course* (create-course (prepare-next 1 '((0 R)) (create-random-course *course-length*))))
	(run-block (+ (car (nth i order-list)) 3)
		   (+ (second (nth i order-list)) 3)
		   (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname)))))))
...
(defun run-experiment-single-mode (n mode)
  ...
  (setf *debg* nil)
  (if (equal mode "auto")
      (prepare-output-file (format nil "~AR/actrtrust/output-auto.txt" (user-homedir-pathname)))
      (prepare-output-file (format nil "~AR/actrtrust/output-manual.txt" (user-homedir-pathname))))
  ...
  (dotimes (k n)
    ...
    (dotimes (j 5)
	  ...
	  (progn
	    ...
	    (pdisable to.manual2) 
	    (setf *auto* 1)
	    (run-block (+ j 3) 3 (format nil "~AR/actrtrust/output-auto.txt" (user-homedir-pathname))))
	  (progn
	    (pdisable to.auto)
	    (pdisable to.auto2)
	    (setf *auto* nil)
	    (run-block 3 (+ j 3) (format nil "~AR/actrtrust/output-manual.txt" (user-homedir-pathname))))))))
...
(defun run-block-single-mode (j mode)
  ...
  (if (equal mode "auto")
	...
	(pdisable to.manual2) 
	(setf *auto* 1)
	(run-block j 3 (format nil "~AR/actrtrust/output-auto.txt" (user-homedir-pathname))))
      (progn
	(pdisable to.auto)
	(pdisable to.auto2)
	(setf *auto* nil)
	(run-block 3 j (format nil "~AR/actrtrust/output-manual.txt" (user-homedir-pathname))))))
...
(defun run-experiment-alpha-egs (n a e th)
  (init)
  (let ((file (format nil "~AR/actrtrust/manip/output~A~A~A.txt" (user-homedir-pathname) a e th)))
...
(defun run-experiment-s-m-r (n &key (s 1) (m 1) (r 1))
  (init)
  (let ((file (format nil "~AR/actrtrust/manip/output~A~A~A.txt" (user-homedir-pathname) s m r)))
    (prepare-output-file file)
    (dotimes (k n)
      (setf *run-num* k)
      (dotimes (i 5)
	(dotimes (j 5)
	  ...
	  (if (equal m 1)
	      (progn
		(pdisable to.auto2) ;with-initial-utility
		(pdisable to.manual2)) ;with-initial-utility
	      (progn
		(pdisable to.auto)	      
		(pdisable to.manual)))
	  (if (equal r 1)
...
(defun run-experiment-thresh-egs (n e th)
  (init)
  (let ((file (format nil "~AR/actrtrust/manip/output~A~A~A.txt" (user-homedir-pathname) 0 e th)))
    (prepare-output-file file)
    (dotimes (k n)
      (setf *run-num* k)
      (dotimes (i 5)
...
;run-experiment-with-order examines order effects of Ca/Cm.
(defun run-experiment-with-order ()
  (init)
  (let ((ac-part (read-capabilities 
		  (format nil "~AR/applications/actr6/capability/auto.lisp" (user-homedir-pathname))))
	(mc-part (read-capabilities 
		  (format nil "~AR/applications/actr6/capability/manual.lisp" (user-homedir-pathname)))))
    (dotimes (i (length ac-part))
      (setf *run-num* i)
      (reset) ;initialize
      (setf *run-time* 0)
      (pdisable to.auto2)	      
      (pdisable to.manual2) 
      ;(setf *auto* (act-r-random 2)) 
      (setf *auto* nil)
      (let ((ac-list (nth i ac-part))
	    (mc-list (nth i mc-part)))
	(setf *course* (create-course (prepare-next 1 '((0 R)) (create-random-course (* *course-length* (length ac-list))))))
	(dotimes (j (length ac-list))
	  (run-block (nth j ac-list) (nth j mc-list) (format nil "~AR/actrtrust/output.txt" (user-homedir-pathname))))))))

(defun read-capabilities (path)
  (let (participant strm)
    (setf strm (open path :direction :input))
    (do ((line1 (read strm nil 'eof)
		(read strm nil 'eof)))
	((eql line1 'eof))
      (setq participant (cons line1 participant)))
    (close strm)
    participant))
...
(defun send-output-file (file)
  (let ((output-stream 
	...
	(uvalue (mapcar #' car (spp-fct '((keep.a keep.m l.to.s r.to.s to.left to.right) :u)))))
...
(sgp 
 ...
 ;:trace-detail medium ;trace detail is selected from "low", "medium" and "high"
 ;:buffer-trace t
...
(p find.goal
   ...
   ?visual>
      state    free
   !bind! =mode (return-current-mode)
   !bind! =conf (car (car (spp (keep.m :u))))
   !bind! =trust (car (car (spp (keep.a :u))))
   ==>
   =goal>
     goal-loc =posi
     current-mode =mode 
...
(spp find.goal :u 0)
(spp keep.m :u 5)
(spp to.left :u 5) ;2019/1/2 0->5
(spp l.to.s :u 5) ;2019/1/2  0->5
(spp to.right :u 5) ;2019/1/2 0->5
(spp r.to.s :u 5) ;2019/1/2 0->5
(spp to.auto :u 5)
(spp to.auto2 :u 5) ;2019/1/4 0->5
(spp keep.a :u 5)
(spp to.manual :u 5)
(spp to.manual2 :u 5) ;2019/1/4 0->5
